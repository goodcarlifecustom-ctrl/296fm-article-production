...
- すべてのH2・H3へ記事内で一意のIDを付ける。IDは後続の`article-linked.html`でも変更しない。
...
導入文の後、最初のH2より前に「この記事でわかること」を通常のdiv、ul、liで1回だけ置いてよい。ただし見出しタグ、H2/H3文字列の転載、ページ内アンカーリンクは使わず、読者が得られる情報を3〜5項目で要約する。装飾工程が標準HTMLのH2リンク一覧を生成するため、`article.html`側では「目次」「INDEX」、[swell_toc]、[toc]、目次用nav、手動のH2/H3アンカーリンク一覧、H2直下のH3一覧、「この章でわかること」を出力しない。H3章内リンクが必要な章は本文へ直接書かず、後工程で`h3_anchor_lists`へ明示する。比較表または要点表、選び方、詳細解説、注意点、FAQ、まとめを検索意図に合わせて自然に含める。
...
- 記事生成時点ではマーカーを本文へ直接書かない。`article-linked.html`完成後、Codexが各H2・H3の直接本文を読んで意味に基づく対象を`decoration.json`へ記入する。
- H3一覧は`article.html`へ追加せず、必要なH2だけを`h3_anchor_lists`で指定して装飾工程に生成させる。
